...
public class Query7 extends QueryObject {
	...
	private void initVariants () {
		...
		variants.add
			...
			"b.PublisherName like '%" + temp[0] + "%' and lb.branchname like '%" + temp[1] + "%'");
		variants.add
			("SELECT b.bookid, b.title, bo.borrowerlname \n" + 
			"FROM (select BookID, title from book where PublisherName like '%" + temp[0] + "%') as b, \n" +
			"book_loans bl, \n" + 
			"(select branchid, branchname from library_branch where branchname like '%" + temp[1] + "%') as lb, \n" + 
			"borrower bo \n" +
			"WHERE b.bookid = bl.bookid and bl.BranchID = lb.BranchID and \n" + 
			"bl.CardNo = bo.CardNo");
		variants.add ("SELECT b.bookid, b.title, bo.borrowerlname \nFROM ((book b natural join book_loans bl) natural join library_branch lb) natural join borrower bo \nWHERE b.PublisherName like '%" + temp[0] + "%' and lb.branchname like '%" + temp[1] + "%'");
		variants.add ("SELECT b.bookid, b.title, bo.borrowerlname \nFROM \n\t(((select BookID, title from book where PublisherName like '%" + temp[0] + "%') as b \n\t\tnatural join book_loans bl) natural join \n\t\t(select branchid, branchname from library_branch where branchname like '%" + temp[1] + "%') as lb) \n\t\tnatural join borrower bo");
		variants.add ("SELECT b.bookid, b.title, bo.borrowerlname \nFROM (select BookID, title from book where PublisherName like '%" + temp[0] + "%') as b, book_loans bl, \n\t(select branchid, branchname from library_branch where branchname like '%" + temp[1] + "%') as lb, borrower bo \nWHERE b.bookid = bl.bookid and bl.BranchID = lb.BranchID and \n\tbl.CardNo = bo.CardNo");
		variants.add ("SELECT b.bookid, b.title, bo.borrowerlname \nFROM \n\t(((select BookID, title from book where PublisherName like '%" + temp[0] + "%') as b \n\tnatural join book_loans bl) \n\tnatural join (select branchid, branchname from library_branch where branchname like '%" + temp[1] + "%') as lb) \n\tnatural join borrower bo");
